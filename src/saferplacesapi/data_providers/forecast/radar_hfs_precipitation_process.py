...
PROCESS_METADATA = {
    ...
    'id': 'safer-process',
    'title': {
        'en': 'Hourly Forecast Precipitation Process',
    },
    'description': {
        'en': 'Hourly Forecast forecast data from Hypermeteo API'
    },
    ...
    'inputs': {
        ...
        'time_delta': {
            'title': 'Time delta',
            'description': 'The time delta in hours. The time delta must be a multiple of 1. If no time delta is provided, the default value is 1 hour',
            'schema': {
            }
        },
        'strict_time_range': {
...
class RadarHFSPrecipitationProcessor(BaseProcessor):
    """Hourly Forecast Precipitation Processor class"""

    def __init__(self, processor_def):
        super().__init__(processor_def, PROCESS_METADATA)
        
        self.dataset_name = 'HFS_ITA_4KM_1H'
        self.variable_name = 'PREC_HOURLY'
    ...
    def validate_parameters(self, data):
        ...
        if time_end is not None:
            if time_start - time_end > datetime.timedelta(days=15):
                raise ProcessorExecuteError('Time range must be less than or equal to 15 days')
    ...
    def retrieve_data(self, auth_token, lat_range, long_range, time_start, time_end=None, timedelta_hours=1):
        ...
        ds = ds.resample(time=f'{timedelta_hours}H').sum()
        return ds
    ...
    def __repr__(self):
        return f'<RadarHFSPrecipitationProcessor> {self.name}'