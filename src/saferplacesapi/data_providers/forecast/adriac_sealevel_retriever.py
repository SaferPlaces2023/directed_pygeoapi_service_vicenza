...
PROCESS_METADATA = {
    ...
    'id': 'safer-process',
    'title': {
        'en': 'Adriac Sea Level Retirever Process',
    },
    'description': {
        'en': 'Retrieve Sea Level data collected from Adriac'
    },
    ...
    'outputs': {
        ...
        'data': {
            ...
            'description': 'Dataset with sea level forecast data time series in requested "out_format"',
            'schema': {
            }
        }
    },
...
class AdriacSeaLevelRetrieverProcessor(BaseProcessor):
    """Adriac Sea Level Retriever process plugin"""
    ...
    def validate_parameters(self, data):
        ...
        time_start = time_start.replace(minute=(time_start.minute // 10) * 10, second=0, microsecond=0)
        time_end = time_end.replace(minute=(time_end.minute // 10) * 10, second=0, microsecond=0) if time_end is not None else time_start + datetime.timedelta(hours=1)
    ...
    def retrieve_adriac_data(self, time_start, time_end):
        s3_filepaths = _s3_utils.list_s3_files(self.bucket_source, filename_prefix=f'{self.dataset_name}__{self.variable_name}')
    ...
    def execute(self, data):
        ...
        try:
            ...
            adriac_dataset = self.retrieve_adriac_data(time_start, time_end)
            
            query_dataset = _processes_utils.dataset_query(adriac_dataset, lat_range, long_range, [time_start, time_end])
    ...
    def __repr__(self):
        return f'<AdriacSeaLevelRetrieverProcessor> {self.name}'