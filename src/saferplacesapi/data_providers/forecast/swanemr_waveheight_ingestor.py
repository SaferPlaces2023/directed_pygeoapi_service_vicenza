...
PROCESS_METADATA = {
    ...
    'id': 'safer-process',
    'title': {
        'en': 'SWANEMR Wave Height Ingestor Process',
    },
    'description': {
        'en': 'Collect Wave Height data from SWANEMR'
    },
    ...
    'inputs': {
        ...
        'forecast_run': {
            'title': 'Forecast run',
            'description': 'SWANEMR forecast runs (optional). If not provided, all the available forecast runs from current date will be considered. The forecast run must be a valid ISO format date string at hour 00:00:00 or 12:00:00 related to at least one week ago',
            'schema': {
    ...
    'example': {
        "inputs": {
            "token": "ABC123XYZ666",
            "debug": True,
            "forecast_run": ["2025-02-26T00:00:00", "2025-02-27T00:00:00"]
        }
    }
}
...
class SwanemrWaveHeightIngestorProcessor(BaseProcessor):
    """Swanemr wave process plugin"""
    ...
    def __init__(self, processor_def):
        ...
        self._data_provider_service = 'https://dati-simc.arpae.it/opendata/swanemr'
    ...
    def get_avaliable_forecast_runs(self):
        ...
        runs = runs[::-1]
        request_file_names = self.get_swanemr_data_filenames(runs)
    ...
    def ping_avaliable_runs(self, forecast_datetime_runs):
        request_file_names = self.get_swanemr_data_filenames(forecast_datetime_runs)
    ...
    def get_swanemr_data_filenames(self, forecast_datetime_runs):
        request_files = []        
        for fdr in forecast_datetime_runs:
            datetime_run_file = f'swanemr.{fdr.year}{fdr.month:02d}{fdr.day:02d}{fdr.hour:02d}00.grib'
            request_files.append(datetime_run_file)
        return request_files
    
    
    def download_swanemr_data(self, forecast_datetime_runs):
        request_file_names = self.get_swanemr_data_filenames(forecast_datetime_runs)
        swanemr_file_paths = []
        for rf in request_file_names:
            ...
            if response.status_code == 200:
                ...
                swanemr_file_paths.append(rf_filename)
            else:
                print(f'Request error {response.status_code} with file "{rf}"')
        return swanemr_file_paths
        
    
    def swanemer_time_concat(self, grib_dss):
        dss = []
        for ids, grib_ds in enumerate(grib_dss):

            # se ci sono altri dataset giornalieri sucessivi prendo solo prime 24 h altrimenti tutto il forecast disponibile
            gmsg = list(grib_ds)[:24] if ids < len(grib_dss)-1 else list(grib_ds)[:72]
            ...
            ds = xr.Dataset(
                {
                    self.variable_name: (["time", "lat", "lon"], np_dataset)  # Nome della variabile e dati
                },
                coords={
                    "time": times_range,
                    "lat": lat_range,
    ...
    def execute(self, data):
        ...
        try:
            ...
            swanemr_file_paths = self.download_swanemr_data(forecast_datetime_runs)
            
            gribs = [pygrib.open(sfp) for sfp in swanemr_file_paths]
            
            timeserie_dataset = self.swanemer_time_concat(gribs)
        ...
        _processes_utils.garbage_filepaths(swanemr_file_paths)
        
        return mimetype, outputs

    def __repr__(self):
        return f'<SwanemrWaveHeightIngestorProcessor> {self.name}'
